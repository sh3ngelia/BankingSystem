...
public class UnfreezeCardCommandHandler : ICommandHandler<UnfreezeCardCommand>
{
    ...
    public async Task<Result> Handle(UnfreezeCardCommand command, CancellationToken ct)
    {
        ...
        await _eventBus.PublishAsync(new CardUnfrozenEvent(
            card.Id,
            card.AccountId,
            DateTime.UtcNow), ct);
